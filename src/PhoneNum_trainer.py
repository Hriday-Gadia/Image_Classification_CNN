...
module_path = "../src/PhoneNum_model.py"
module_name = "PhoneNum_model"

spec = importlib.util.spec_from_file_location(module_name, module_path)
phone_module = importlib.util.module_from_spec(spec)
sys.modules[module_name] = phone_module
spec.loader.exec_module(phone_module)

with open("../src/configphone.json", "r") as f:
    config = json.load(f)
...
# Load the saved data and config
bal_data , bal_label  = torch.load("../src/Dataset/PhoneNum/PhoneNum_balanced.pt",weights_only=True)
imbal_data , imbal_label = torch.load("../src/Dataset/PhoneNum/PhoneNum_imbalanced.pt",weights_only=True)

model1 = phone_module.ModularMLP(model_params["input_size"],model_params["activation"],model_params["hidden_layers"],model_params["output_size"])
model2 = phone_module.ModularMLP(model_params["input_size"],model_params["activation"],model_params["hidden_layers"],model_params["output_size"])
...
def test_model(model, images, labels, epochs, device="cpu"):
    ...
    for epoch in range(epochs):
        ...
        for batch_imgs, batch_labels in loader:
            ...
            partitioned_images = []
            partition_height = sample.shape[0]
            partition_width = sample.shape[1] // 12
